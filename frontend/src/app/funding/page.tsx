...
import { FundingWorkspaceClient } from '../../components/funding/FundingWorkspaceClient';
import { ApiResponse } from '../../types';

const API_BASE = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001/api/v1';

async function getCompanies(): Promise<{ items: any[]; total: number; cursor: string | null }> {
  const res = await fetch(`${API_BASE}/companies?limit=30&sort=funded`, { next: { revalidate: 60 } });
  if (!res.ok) throw new Error('Failed to fetch companies list for funding');
...
export default async function FundingPage() {
  try {
    ...
    console.error('Funding Workspace Server Fetch Error:', error);
    return (
      <div className="flex flex-col min-h-screen">
        <Navbar />
        <main className="flex-grow flex items-center justify-center p-8 bg-slate-50/50">
          <div className="max-w-md w-full text-center bg-white border border-gray-200 p-8 rounded-2xl shadow-sm">
            ...
            <p className="text-sm text-gray-500 mb-6">
              Could not retrieve funding directory. Please ensure the backend server is running and database seed is completed.
            </p>
            <a 
              href="/funding" 
